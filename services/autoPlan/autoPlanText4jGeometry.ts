...
// Isomorphic half of the Text 4.0 J Auto Plan adapter: the prompt builder and the
// GeneratedData -> Auto Plan payload conversion, with no Node-only imports. The browser
// pipeline (autoPlanText4jClient) and the serverless fallback (backend/text4jAutoPlanAdapter)
// both reuse this so the two paths cannot drift apart.
...
export const buildAutoPlanImagePrompt = (boundary: AutoPlanBoundary, brief: AutoPlanBrief): string => {
...
// The local extractor names spaces it actually finds in the raster; it reads the brief as
// free text, the same way Text 4.0 J passes its design summary through.
export const buildAutoPlanDesignSummary = (brief: AutoPlanBrief): string => {
  const roomList = brief.rooms.map(r => `${r.count}x ${r.type}`).join(', ');
  return [
    `${brief.residentialType} residential floor plan.`,
...
export const generatedDataToAutoPlanPayload = (
  ...
  brief: AutoPlanBrief,
  metadataOverrides: Partial<AutoPlanMetadata> = {},
): AutoPlanImportPayload => {
